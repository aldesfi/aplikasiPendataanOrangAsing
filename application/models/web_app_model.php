...
class Web_App_Model extends CI_Model {
	...
	public function getAllDataLimitedMasuk($table,$offset,$limit)
	{
		return $this->db->query('SELECT * FROM tbl_orang_asing WHERE status = "0"');
	}
	
  	public function getAllDataLimitedData()
	{
		return $this->db->query('SELECT *,countries.langEN as asal_negara,(CASE 
        WHEN tbl_orang_asing.jenis = "0" THEN  "-"
       WHEN tbl_orang_asing.jenis = "1" THEN  "Perusahaan Dalam Negeri"
               WHEN tbl_orang_asing.jenis = "2" THEN  "Perusahaan Dalam Negeri"
               WHEN tbl_orang_asing.jenis = "3" THEN  "Perusahaan Dalam Negeri"
            
    END) AS jenis


 FROM tbl_orang_asing join countries 
    where countries.alpha2=tbl_orang_asing.asal_negara');
	}
  
   	public function getAllDataLimiteUpdate($table,$offset,$limit)
	{
	return $this->db->query('SELECT * FROM tbl_orang_asing WHERE status = "1"');
	}
  
  
     	public function getListDataOrangAsingBekerja()
	{
	return $this->db->query('SELECT * FROM `tbl_orang_asing` where status_pekerjaan=1');
	}
  
       	public function getListDataOrangAsingTidakBekerja()
	{
	return $this->db->query('SELECT * FROM `tbl_orang_asing` where status_pekerjaan=0');
	}
  
  
  
    	public function getAllDataLimitedDataPilih($id_orang_asing)
	{
		return $this->db->query('SELECT *,countries.langEN as asal_negara,(CASE 
       WHEN tbl_orang_asing.jenis <= "1" THEN  "Perusahaan Dalam Negeri"
               WHEN tbl_orang_asing.jenis <= "2" THEN  "Perusahaan Dalam Negeri"
 ...
 FROM tbl_orang_asing join countries 
    where countries.alpha2=tbl_orang_asing.asal_negara and tbl_orang_asing.id_orang_asing="'.$id_orang_asing.'"');
	}
     	...
     	public function getAllDataOrangAsingBekerja()
	{
	return $this->db->query('SELECT countries.langEN as asal_negara,sum(case when jenis=1 then 1 else 0 end ) as dalam_negeri, tbl_orang_asing.asal_negara as kode_negara,
sum(case when jenis=2 then 1 else 0 end ) as luar_negeri, 
sum(case when jenis=3 then 1 else 0 end ) as lembaga, count(asal_negara) as jumlah

FROM `tbl_orang_asing` join  countries where status_pekerjaan=1 AND countries.alpha2=tbl_orang_asing.asal_negara group by asal_negara ');
	}
  
       	public function getAllDataOrangAsingTidakBekerja()
	{
	return $this->db->query('SELECT countries.langEN as asal_negara, count(asal_negara) as jumlah

FROM `tbl_orang_asing` join  countries where status_pekerjaan=0     AND countries.alpha2=tbl_orang_asing.asal_negara group by asal_negara ');
	}
			...
			foreach($q_cek_login->result() as $qck)
			{
				if($qck->stts=='perusahaan')
				{
					...
					foreach($q_ambil_data ->result() as $qad)
					{
						...
						$sess_data['stts'] = 'perusahaan';
						$this->session->set_userdata($sess_data);
					}
					header('location:'.base_url().'perusahaan');
				}
				else if($qck->stts=='pemantau')
				{
						$q_ambil_data = $this->db->get_where('tbl_admin', array('username' => $u));
					foreach($q_ambil_data ->result() as $qad)
					{
